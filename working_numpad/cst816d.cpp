 ...
 * @FilePath: \PlatformIO\src\f6336u_device.cpp
 * @Description: 
 * 
...
bool touch_init(int tp_sda, int tp_scl, int tp_rst, int tp_int)
{
    ...
    uint8_t data[5] = {0};
    touch_i2c_read(CST816D_ADDR, CST816D_ID_REG, data, 1);
    if (data[0] == 0xB6){
        Serial.printf("Find CST816T!\n");
    }
    return true;
}
...
bool get_touch_coords(toucht_coords_t *toucht_coords)
{
    ...
    touch_i2c_read(CST816D_ADDR, CST816D_TOUCH_NUM_REG, &touch_num, 1);
    if (touch_num == 0)
        return false;
    

    touch_i2c_read(CST816D_ADDR, CST816D_TOUCH_XH_REG, x_arr, 1);
    touch_i2c_read(CST816D_ADDR, CST816D_TOUCH_XL_REG, x_arr+1, 1);

    touch_i2c_read(CST816D_ADDR, CST816D_TOUCH_YH_REG, y_arr, 1);
    touch_i2c_read(CST816D_ADDR, CST816D_TOUCH_YL_REG, y_arr+1, 1);
